...
- Fix approach: Move shared utilities into a `Duncedmaxxing/Util.lua` file loaded before `Core.lua` and `Options.lua`, exposing them on the `DMX` table or as upvalues shared via the addon table.
