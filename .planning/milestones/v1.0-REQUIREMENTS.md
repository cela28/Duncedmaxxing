...
- [x] **PERF-02**: Spell texture resolved and cached once at Initialize time (and on PLAYER_LOGIN), not on every Update and RefreshLayout call — *Complete → later retired in Phase 7. Icon-mode removal in Phase 5 orphaned the texture cache (its sole consumer); Phase 7 deleted the dead code. The requirement was satisfied and tested at Phase 4; its implementation no longer exists.*
...
| PERF-02 | Phase 4 | Complete → retired (Phase 7) |
